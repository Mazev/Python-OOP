class PizzaDelivery:
    ...
    def add_extra(self, ingredient, quantity, ingredient_price: object):
        if self.ordered:
    ...
    def print_prepared(self):
        return ", ".join([f"{key}: {value}" for key, value in self.ingredients.items()])

    def make_order(self):
        self.ordered = not self.ordered
        return f"You've ordered pizza {self.name} prepared with {self.print_prepared()} and the price will be {self.price}lv."
