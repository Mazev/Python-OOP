class Vet:
    ...
    def unregister_animal(self, animals_name) -> object:
        if animals_name not in self.animals:
            return f"{animals_name} not in the clinic"

        Vet.animals.remove(animals_name)
        self.animals.remove(animals_name)
        return f"{animals_name} unregistered successfully"

    def info(self):
        vet_animal_space = len(self.animals)
        clinic_animal_count = len(Vet.animals)
        space_left = Vet.space - clinic_animal_count
        return f"{self.name} has {vet_animal_space} animals. {space_left} space left in clinic"
