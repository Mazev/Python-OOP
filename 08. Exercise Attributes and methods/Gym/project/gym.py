class Gym:
    ...
    def subscription_info(self, subscription_id):
        curen_sub = [s for s in self.subscriptions if s.id == subscription_id][0]
        customer = [c for c in self.customers if curen_sub.customer_id == c.id][0]
        trainer = [t for t in self.trainers if t.id == curen_sub.trainer_id][0]
        plan = [p for p in self.plans if p.trainer_id == trainer.id][0]
        equp = [e for e in self.equipment if e.id == plan.equipment_id][0]
